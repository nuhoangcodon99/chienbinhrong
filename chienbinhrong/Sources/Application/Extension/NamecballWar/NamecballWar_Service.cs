...
namespace NgocRongGold.Application.Extension.NamecballWar
{
    public class NamecballWar_Service
    {
                ...
                case NamecballWar_Team.Fide:
                    {
                        ...
                        {
                            var character = ClientManager.Gi().GetCharacter(id);
                            if (character != null) character.CharacterHandler.SendMessage(msg);
                        });
                    }
                    break;
        ...
        public static void Send_ChienTruong_Namek(Character character, int type, params int[] column)
        {

            switch (type)
            {
                case 0:
                    character.CharacterHandler.SendMessage(newInfoPhuBan(164, "Ca Đíc", "Fide", 7, 900, 7));
                    break;
                case 1:
                    character.CharacterHandler.SendMessage(updatePoint(column[0], column[1]));
                    break;
                case 2:
                    character.CharacterHandler.SendMessage(AddEffectEnd(column[0]));
                    break;
                case 5:
                    character.CharacterHandler.SendMessage(updateTime((short)column[0]));
                    break;
                default:
                    character.CharacterHandler.SendMessage(updateLife((byte)column[0], (byte)column[1]));
                    break;
            }
            Server.Gi().Logger.Print("Chien truong namec msg: 20 type: " + type);
        }
