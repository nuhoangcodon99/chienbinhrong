...
namespace NgocRongGold.Application.Extension.Bosses.SoiHecQuyn
{
    public class TrongTaiHandler : ICharacterHandler
    {
        ...
        public TrongTaiHandler(Boss boss)
        {
            Boss = boss;
        }
        ...
        private void AutoBoss(long timeServer)
        {
            ...
            if (Boss.Zone.Map.Id is 113) return;
        ...
        {
            var current = TextServer.gI().TRONG_TAI;
            SendBossChat(current[ServerUtils.RandomNumber(current.Count)]);
