...
namespace NgocRongGold.Application.MainTasks
{
    public class TaskHandler
    {
        ...
        public Message UpdateIndex(Character character)
        {
            var msg = new Message(41);
            character.CharacterHandler.SendMessage(msg);
            return msg;
        }
        public void UpdateTask(Character character) // load lại nhiệm vụ 
        {
            character.CharacterHandler.SendMessage(Service.SendTask(character));
        }

        public Message UpdateCount(Character character)
        {
        ...
        public void RewardDoneTask(Character character)
        {

        }

        public void CheckTaskDoneKillMob(Character character, MonsterMap monster)
        {
        ...
        public string Replace(Character character, string text)
        {
            switch (text)
            {
                case "#vachnui":
        ...
        public void Message(Character character)
        {
        ...
        public Boolean ReportTask(Character character, int npcId)
        {
            ...
            switch(character.InfoTask.Id, character.InfoTask.Index, npcId)
            {
                ...
                character.Zone.ZoneHandler.AddBoss(tau77);
        ...
        public void PlusSubTask(Character character, int plusCount)
        {
        ...
        public void SendNextTask(Character character)
        {
            ...
            switch (bossType, character.InfoTask.Id, character.InfoTask.Index)
            {
                case (47, 20, 1):
                    ...
                    if (clan != null)
                    {
                        ...
                        {
                            var temp = ClientManager.Gi().GetCharacter(clan.Thành_viên[i].Id);
                            if (temp != null && DataCache.IdMapReddot.Contains(temp.InfoChar.MapId))
                            {
                                if (CheckTask((Character)temp, 20, 1))
                                {
                                    PlusSubTask((Character)temp, 1);
                                }
                            }
                        }
                    }
        ...
        public void CheckTaskMoveInMap(Character character)
        {
        ...
        public void CheckTaskDoneGoToMap(Character character)
        {
            switch(character.InfoChar.MapId, character.InfoTask.Id, character.InfoTask.Index)
            {
        ...
        public void CheckTaskDonePower(Character character)
        {
        ...
        public void DoSendMessage(Character character, string message) // Gửi Thông Báo Extension Message
        {
            character.CharacterHandler.SendMessage(Service.ServerMessage(message));
        }
        public void DoUISay(Character character, string say, short npcId = 5) // mở Menu Ui Con Mèo
        {
            character.CharacterHandler.SendMessage(Service.OpenUiSay(npcId, $"{say}"));
        }

        public void LoadInfo(Character character) // send lại info nhân vật như vàng , ngọc
        {
            character.CharacterHandler.SendMessage(Service.BuyItem(character));
        }

        public void CongTiemNangSucManh(Character character, long gold, long gem, long tnsm)
        {
