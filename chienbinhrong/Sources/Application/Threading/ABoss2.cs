...
/// Code by baodubai dep trai vai lon
            private static ABoss2 Instance { get; set; } = null;
            public bool IsStop = false;
            public static ABoss2 gI()
            {
                return Instance ??= new ABoss2();
            }
            ...
            public void Start()
            {
            Runtime = new Task(AutoBossAsync);
            Runtime.Start();

            }
            public async void AutoBossAsync()
            {
                        ...
                        if (DelayBroly < timeserver)
                        {
                            ...
                            else
                            {
                                for (int i = 0; i < ListBroly.Count; i++)
                                {
                                    ...
                                    Output($"Broly ReSpawn: [Mapid: {ListBroly[i].Zone.Map.Id}, Zone: {ListBroly[i].Zone.Id}], CountBroly: {countBroly}");
