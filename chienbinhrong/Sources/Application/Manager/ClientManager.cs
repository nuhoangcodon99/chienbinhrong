...
namespace NgocRongGold.Application.Manager
{
    public class ClientManager : IClientManager
    {
        ...
        public long DelayUpdateSession = 180000 + ServerUtils.CurrentTimeMillis();
        public Task varUpdateSession { get; set; }
        public void StartUpdateSession()
        {
            varUpdateSession = new Task(UpdateSession);
            varUpdateSession.Start();
        }
        ...
        public async void UpdateSession()
        {
            while (Server.Gi().IsRunning)
            {
                ...
                if (DelayUpdateSession < timeServer)
                {
                    foreach (var session in Sessions.Values.Where(ss => !ss.IsLogin && timeServer - ss.TimeConnected >= 60000))
                    {
