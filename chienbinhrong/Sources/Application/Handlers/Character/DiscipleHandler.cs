    ...
    public class DiscipleHandler : ICharacterHandler
    {
            ...
            lock (Disciple)
            {
                try
                {
                    ...
                    if ((Disciple.InfoChar.IsDie || Disciple.InfoChar.Hp == 0) && Disciple.InfoDelayDisciple.LeaveDead <= timeServer)
                    {
                        ...
                        if (Disciple.Character.Zone != null && !DataCache.IdMapKarin.Contains(Disciple.Character.InfoChar.MapId))
                        {
                            Disciple.Character.Zone.ZoneHandler.AddDisciple(Disciple);
                ...
                {
                    Manager.ClientManager.Gi().KickSession(Disciple.Character.Player.Session);
                    Server.Gi().Logger.Error($"Error UpdateDisciple in Service.cs: {e.Message} \n {e.StackTrace}", e);
                    File.AppendAllText("error disciple//" + Disciple.Id + ".ini", e.Message + "\n" + e.StackTrace + "\n\n----------------------\n\n");
                }
        ...
        public void PlusTiemNang(IMonster monster, int damage)
        {
            ...
            if (monster.Id != 0)
            {
                ...
                //    damagePlusPoint = 1;
                //}
                if (monster.Id is (77 or 70 or 71 or 85 or 72 or 76))
                {
                    damagePlusPoint = 1;
                }
