...
namespace NgocRongGold.Application.Handlers.Client
{
    public static class GachThe
    {
        ...
        {
            string partnerId = "88374668836";
            string partnerKey = "ce898aa23be13c6a12d4f7ab334a0555";
            string url = "https://vipcard24h.com/chargingws/v2";
            ...
            try
            {
                ...
                if (jsonResult.status == 1) {
                    character.CharacterHandler.SendMessage(Service.ServerMessage("Con đã nạp thẻ thành công. Đợi một vài phút rồi hãy đến gặp Quý Ngài Santa để đổi vàng nhé."));
                }
                else if (jsonResult.status == 2) {
                    character.CharacterHandler.SendMessage(Service.ServerMessage( "Con đã nạp thẻ thành công. Những sai mệnh giá, con sẽ bị trừ 50% tiền phí nhé. Đợi một vài phút rồi hãy đến gặp Quý Ngài Santa để đổi vàng nhé."));
                }
                else if (jsonResult.status == 3) {
                    character.CharacterHandler.SendMessage(Service.ServerMessage("Thẻ của con bị lỗi rồi."));
                    return;
                }
                else if (jsonResult.status == 4) {
                    character.CharacterHandler.SendMessage(Service.ServerMessage("Hệ thống đang bảo trì rồi con à."));
                    return;
                }
                else if (jsonResult.status == 99) {
                    character.CharacterHandler.SendMessage(Service.ServerMessage("Đã gửi thẻ thành công, con hãy đợi một lát để hệ thống xử lý nhé."));
                }
                else if (jsonResult.status == 100) {
                    character.CharacterHandler.SendMessage(Service.ServerMessage("Con đã nhập sai mã pin và seri, hãy kiểm tra và nhập đúng Seri ở trên và Mã thẻ ở dưới nhé."));
                }
                else {
                    character.CharacterHandler.SendMessage(Service.ServerMessage(jsonResult.message));
                    return;
                }
