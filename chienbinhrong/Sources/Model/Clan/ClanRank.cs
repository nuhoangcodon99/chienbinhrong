        ...
        public static void SelectTopKhiGas(int limit)
        {
            ...
            lock (Server.SQLLOCK)
            {
                try
                {
                    ...
                    command.CommandText = $"SELECT `Name`, `LeaderName`,JSON_EXTRACT(Leader, '$.Head'), JSON_EXTRACT(Leader, '$.Body'), JSON_EXTRACT(Leader, '$.Leg'), JSON_EXTRACT(ClanDungeon, '$.HighScore') AS HighScore, JSON_EXTRACT(ClanDungeon, '$.TimeSetHighScore') AS TimeSetHighScore, JSON_EXTRACT(ClanDungeon, '$.LevelScore') FROM `clan` ORDER BY HighScore,JSON_EXTRACT(ClanDungeon, '$.LevelScore'),TimeSetHighScore DESC LIMIT 100";
