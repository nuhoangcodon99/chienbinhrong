...
namespace NgocRongGold.DatabaseManager.Player
{
    public class UserDB
    {
        public static long GetTimeOut(string username)
        {
            lock (Server.SQLLOCK)
            {
                try
                {
                    ...
                    {
                        command.CommandText =
                            $"SELECT `logout_time` FROM `user` WHERE `username` = '{username}' LIMIT 1;";
        ...
        public static long GetTimeOut(int id)
        {
            lock (Server.SQLLOCK)
            {
                try
                {
                    ...
                    {
                        command.CommandText =
                            $"SELECT `logout_time` FROM `user` WHERE `id` = '{id}' LIMIT 1;";
                        using (var reader = command.ExecuteReader())
                        {
                            if (reader.HasRows)
                            {
                        ...
                        using (var reader = command.ExecuteReader())
                        {
                            if (reader.HasRows)
                            {
                                while (reader.Read())
                                {
                                    ...
                                    //List<int> Players = JsonConvert.DeserializeObject<List<int>>(reader.GetString(3));
                                    var Players = (reader.GetInt32(3));
                                    int role = reader.GetInt32(5);
                                    byte ban = reader.GetByte(6);
                                    int tongVND = reader.GetInt32(11);
                                    int tichnap = reader.GetInt32(12);
                                    //var characters = new List<Character>();
                                    //foreach (var pl in Players)
                                    //{
                                    //    if (pl == 0) characters.Add(null);
                                    //    else characters.Add(CharacterDB.GetById(pl));
                                    //}
        ...
        public static List<Character> SelectCharacters(string username, string password)
        {
            lock (Server.SQLLOCK)
            {
                try
                {
                    ...
                    {
                        command.CommandText =
                            $"SELECT `character` FROM `user` WHERE (`username` = '{username}' AND `password` = '{password}');";
                        using (var reader = command.ExecuteReader())
                        {
                            if (reader.HasRows)
                            {
                                ...
                                {
                                    var characters = new List<Character>();
                                    var ids = JsonConvert.DeserializeObject<List<int>>(reader.GetString(0));
                                    foreach(var id in ids)
                                    {
                                        if (id == 0) characters.Add(null);
                                        else
                                        {
                                            characters.Add(CharacterDB.GetById(id));
                                        }
                                    }
                                    return characters;
                                }
        ...
        public static bool CreateUser(string username, string password, int characterId, int role =0)
        {
            lock (Server.SQLLOCK)
            {
                try
                {
                    ...
                    {
                        command.CommandText =
                            $"INSERT INTO `user` (`username`, `password`, `character`, `role`) VALUES ('{username}', '{password}', '0', '{role}')";
        ...
        public static void UpdateUser(string usernameOld, string passwordOld, string usernameNew, string passwordNew)
        {
            lock (Server.SQLLOCK)
            {
                try
                {
                    ...
                    {
                        // int port = DatabaseManager.Manager.gI().ServerPort;
                        command.CommandText = $"UPDATE `user` SET `username`= '{usernameNew}', `password`= '{passwordNew}' WHERE `username` = '{usernameOld}' AND `password` = '{passwordOld}' LIMIT 1;";
                        command.ExecuteNonQuery();
                    }
                }
                catch (Exception e)
        ...
        public static bool Update(Model.Player player, string ipV4 = "", bool isCreateChar = false)
        {
            if (ipV4 != "")
            {
                ...
                //if (ClientManager.Gi().UserLoginTime.TryGetValEe(player.Id, out thoiGianDangNhap) && !isCreateChar)
                //{
        ...
        public static bool UpdatePrivateGit(int userId,string gitcode = "")
        {
            lock (Server.SQLLOCK)
            {
                try
                {
                    ...
                    using (DbCommand command = DbContext.gI()?.Connection.CreateCommand())
                    {
                        command.CommandText = $"UPDATE `user` SET `private_gitcode`= '{gitcode}' WHERE `id`=" + userId + " LIMIT 1;";
                        command.ExecuteNonQuery();
                        return true;
                    }
                }
        ...
        public static string GetPrivateGitcode(Model.Player player)
        {
            lock (Server.SQLLOCK)
            {
                try
                {
                    ...
                    {
                        command.CommandText =
                            $"SELECT `private_gitcode` FROM `user` WHERE `id` = '{player.Id}' LIMIT 1;";
                        using (var reader = command.ExecuteReader())
                        {
                            if (reader.HasRows)
                            {
