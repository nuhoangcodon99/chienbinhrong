        ...
        public static int Create(Clan clan)
        {
            lock (Server.SQLLOCK)
            {
                try
                {
                    ...
                    {
                        command.CommandText =
                            $"INSERT INTO `clan` (`Name`, `Khẩu hiệu`, `ImgId`, `Điểm thành tích`, `LeaderName`, `Thành viên hiện tại`, `Thành viên tối đa`, `Thời gian tạo bang`, `Thành viên`, `Messages`, `CharacterPeas`, `DataBlackBall`, `Leader`, `ClanBox`,`Điểm Danh Vọng`, `ClanDungeon`,`DateTime`,`shortName`) VALUES ('{clan.Name}', '{clan.Khẩu_hiệu}', {clan.ImgId}, {clan.Điểm_thành_tích}, '{clan.LeaderName}', {clan.Thành_viên_hiện_tại}, {clan.Tối_đa_thành_viên}, {clan.Thời_gian_tạo_bang}, '{JsonConvert.SerializeObject(clan.Thành_viên)}', '{JsonConvert.SerializeObject(clan.Messages)}' , '{JsonConvert.SerializeObject(clan.CharacterPeas)}','{JsonConvert.SerializeObject(clan.DataBlackBall)}', '{JsonConvert.SerializeObject(clan.Leader)}','[]','{clan.Điểm_Danh_Vọng}', '{JsonConvert.SerializeObject(clan.ClanDungeon)}', '{clan.DateClanCreate:yyyy-MM-dd HH:mm:ss}', ''); SELECT LAST_INSERT_ID();";
        ...
        public static void Update(Clan clan)
        {
            lock (Server.SQLLOCK)
            {
                try
                {
                    ...
                    text += $", `ClanDungeon` = '{JsonConvert.SerializeObject(clan.ClanDungeon)}'";
